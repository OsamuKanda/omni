Partial Public Class OMN613Dao(Of T)
    ...
    Public Function gBlnGetSEIKYUKING(ByVal o As ClsOMN613) As String
        ...
        With o.gcol_H
            ...
            strSQL.Append(" SELECT (SUM(DT_URIAGEM.KING) + (CASE WHEN DT_URIAGEH.SEIKYUYMD < '20231001' THEN SUM(DT_URIAGEM.TAX) ELSE DECODE(DT_URIAGEH.TAXKBN,'0',ROUND(SUM(DT_URIAGEM.KING)/10),0) END)) AS GOKEI ")
            strSQL.Append(" FROM ")
            strSQL.Append(" DT_URIAGEM, DT_URIAGEH ")
            strSQL.Append(" WHERE DT_URIAGEM.DELKBN = '0'")
            strSQL.Append("   AND DT_URIAGEM.SEIKYUSHONO = DT_URIAGEH.SEIKYUSHONO") '請求書番号
            strSQL.Append("   AND DT_URIAGEM.SEIKYUSHONO = '" & .strSEIKYUSHONO & "'") '請求書番号
    ...
    Public Function gBlnGetTAX(ByVal o As ClsOMN613) As String
        Dim ds As New DataSet
        Dim strSQL As New StringBuilder
        With o.gcol_H
            '★消費税の計算は明細毎でなく明細の合計に対して実施する
            ...
            'strSQL.Append("   AND DT_URIAGEM.SEIKYUSHONO = '" & .strSEIKYUSHONO & "'") '請求書番号
            'strSQL.Append(" GROUP BY DT_URIAGEM.SEIKYUSHONO ")
            strSQL.Append(" SELECT (CASE WHEN DT_URIAGEH.SEIKYUYMD < '20231001' THEN SUM(DT_URIAGEM.TAX) ELSE DECODE(DT_URIAGEH.TAXKBN,'0',ROUND(SUM(DT_URIAGEM.KING)/10),0) END) AS TAX ")
            strSQL.Append(" FROM ")
            strSQL.Append(" DT_URIAGEM, DT_URIAGEH ")
            strSQL.Append(" WHERE DT_URIAGEM.DELKBN = '0'")
            strSQL.Append("   AND DT_URIAGEM.SEIKYUSHONO = DT_URIAGEH.SEIKYUSHONO") '請求書番号
        ...
        mclsDB.gBlnFill(strSQL.ToString, ds)
        mclsDB.gBlnDBClose()
        Dim tax = ds.Tables(0).Rows(0).Item("TAX").ToString
        Return tax
    End Function
