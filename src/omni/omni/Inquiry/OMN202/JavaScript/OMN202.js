...
function submitBefor() {
	var SMode = document.getElementById(SerachMode);
	// hidden属性のセット
	if (SMode.value != '') {
	    close();
	    return false;
...
function AJCon(strID, nNowID) {
	...
	switch (strAJ) {
  		...
  		case "btnAJNONYUNMR01":
		case "btnAJTANTNM":
		case "btnAJNONYUNMR02":
		case "btnAJHOKOKUSHOKBN":
			bRet = btnAJOne(strAJ);
			break;
...
function NONYUCD_Search(e, str) {
	...
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJNONYUNMR01' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function SEIKYUCD_Search(e, str) {
	...
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJNONYUNMR02' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
