...
function setScroll(nNum) {
    if (scrollCHG) {
        ...
        if (nNum == 2) {
            if (scrollElm.scrollHeight > 121) {
                scrollElm.scrollTop = scrollElm.scrollHeight - 119;
            } else {
                scrollElm.scrollTop = scrollVal;
            }
        } else {
            if (scrollElm.scrollHeight > 121) {
                if ((scrollElm.scrollHeight - 120) < scrollVal) {
                    scrollElm.scrollTop = scrollElm.scrollHeight - 119;
                } else {
                    scrollElm.scrollTop = scrollVal;
                }
...
function NONYUCD_Search(e, str) {
	...
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJNONYUNM' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function SEIKYUCD_Search(e, str) {
    ...
    var elmNo2 = getElmNo('SEIKYUCD' + str);
    hidstrFocusSet(elmNo, elmNo2, '0');
    var elm = getNextFocusElm(elmNo, 'cli');
    var elm2 = getNextFocusElmE(elmNo2, 'cli');
    //modori値の処理
    e.disabled = false;
    if (retval != null) {
        if (btnAJOne('btnAJSEIKYUNM' + str) == false) {
            elm.focus();
        }
    } else {
        elm2.focus();
...
function SEIKYUSHONO_Search(e, str) {

    var url = "../../../Search/OMN824/Contents/OMN824.aspx?MODE=OMN601";
	var mode = "Search";
	var nWidth = "917px";
	var nHeight = "665px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	if (retval != null) {
		for (i = 0; i < objclient.length; i++) {
		    if (objclient[i][1] == "SEIKYUSHONO" + str) {
				elm = document.getElementById(objclient[i][0]);
				elm.value = retval[0].rtrim();
				elm.style.backgroundColor = objelmdata.Color('FocusOFF');
			}
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnSEIKYUSHONO' + str);
	var elmNo2 = getElmNo('SEIKYUSHONO' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
...
function HINCD_Search(e, str) {

	var url = "../../../Search/OMN822/Contents/OMN822.aspx";
	var mode = "Search";
	var nWidth = "441px";
	var nHeight = "567px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnHINCD' + str);
	var elmNo2 = getElmNo('HINCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJHINNM1' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
