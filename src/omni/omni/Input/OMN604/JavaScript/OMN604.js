...
function setScroll(nNum) {
    if (scrollCHG) {
        ...
        if (nNum == "1") {
            if (scrollElm.scrollHeight > 241) {
                scrollElm.scrollTop = scrollElm.scrollHeight - 239;
            } else {
                scrollElm.scrollTop = scrollVal;
            }
        } else {
            if (scrollElm.scrollHeight > 241) {
                if ((scrollElm.scrollHeight - 241) < scrollVal) {
                    scrollElm.scrollTop = scrollElm.scrollHeight - 239;
                } else {
                    scrollElm.scrollTop = scrollVal;
                }
...
function AJCon(strID, nNowID) {
	...
	switch (strAJ) {
		...
		case "btnAJTANTNM":
		case "btnAJNum00":
		case "btnAJBBUNRUINM00":
		case "btnAJTANINM00":
		case "btnAJBKIKAKUNM00":
		case "btnAJJIGYOCD00":
		case "btnAJSAGYOBKBN00":
		case "btnAJRENNO00":
		case "btnAJModeCng":
		case "btnAJNext":
		case "btnAJF2":
...
function HACCHUNO_Search(e, str) {
    var Qu = '?';
    Qu += 'VIEWID=OMN604';
    Qu += '&Mode=search';
    
	var url = "../../../Search/OMN813/Contents/OMN813.aspx" + Qu;
	var mode = "Search";
	var nWidth = "678px";
	var nHeight = "609px";
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnHACCHUNO' + str);
	var elmNo2 = getElmNo('HACCHUNO' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
...
function SIRCD_Search(e, str) {
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnSIRCD' + str);
	var elmNo2 = getElmNo('SIRCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJSIRNMR' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function TANTCD_Search(e, str) {
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnTANTCD' + str);
...
function RENNO_Search(e, str) {
    ...
    if (retval != null) {
        for (i = 0; i < objclient.length; i++) {
            ...
            if (objclient[i][1] == "SAGYOBKBN" + str) {
                elm = document.getElementById(objclient[i][0]);
	...
	// 規定値のフォーカスセット
    var elmNo = getElmNo('BUKKENNM' + str);
    var elmNo2 = getElmNo('JIGYOCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJRENNO' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function BKIKAKUCD_Search(e, str, strmode) {

	var url = "../../../Search/OMN811/Contents/OMN811.aspx";
	var mode = "Search";
	var nWidth = "753px";
	var nHeight = "588px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	if (retval != null) {
		for (i = 0; i < objclient.length; i++) {
			if (objclient[i][1] == "BBUNRUICD" + str) {
				elm = document.getElementById(objclient[i][0]);
				elm.value = retval[0].rtrim();
				elm.style.backgroundColor = objelmdata.Color('FocusOFF');
			}
			if (objclient[i][1] == "BKIKAKUCD" + str) {
				elm = document.getElementById(objclient[i][0]);
				elm.value = retval[1].rtrim();
				elm.style.backgroundColor = objelmdata.Color('FocusOFF');
			}
	...
	// 規定値のフォーカスセット
	if (strmode == "BUN") {
	    var elmNo = getElmNo('btnBBUNRUICD' + str);
	    var elmNo2 = getElmNo('BBUNRUICD' + str);
	} else {
	    var elmNo = getElmNo('btnBKIKAKUCD' + str);
	    var elmNo2 = getElmNo('BKIKAKUCD' + str);
	}
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	...
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJBKIKAKUNM' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
