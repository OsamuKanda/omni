...
function NYUKINNO_Search(e, str) {
    var elm = document.getElementById(strSEIKYUSHONO);
    var Qu = '?SEIKYUSHONO=';
    if (elm.value != '') {
        Qu += elm.value;
    }

    var url = "../../../Search/OMN827/Contents/OMN827.aspx" + Qu;
    var mode = "Search";
    var nWidth = "488px";
    var nHeight = "609px";
    ...
    // 規定値のフォーカスセット
    var elmNo = getElmNo('btnSearch2' + str);
    var elmNo2 = getElmNo('NYUKINNO' + str);
    hidstrFocusSet(elmNo, elmNo2, '0');
...
function GINKOCD_Search(e, str) {

	var url = "../../../Search/OMN808/Contents/OMN808.aspx";
	var mode = "Search";
	var nWidth = "319px";
	var nHeight = "567px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('TEGATANO' + str);
	var elmNo2 = getElmNo('GINKOCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJGINKONM' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
