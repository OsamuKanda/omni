...
#Region "イベント"
    ...
    Private Sub DefaultSetControl()
        With CType(mprg.gmodel, ClsOMN801).gcol_H
            '事業所コード
            'QueryStringの設定
            Dim strJigyo = Request.QueryString("JIGYOCD")
            Dim strdisable = Request.QueryString("disable")
            JIGYOCD.Focus()
            If strJigyo <> "" Then
                JIGYOCD.SelectedValue = ClsEditStringUtil.gStrConvSelectedValue(strJigyo, JIGYOCD)
                If JIGYOCD.SelectedValue <> "" AndAlso strdisable = "true" Then
                    mprg.mwebIFDataTable.gSub項目有効無効設定("JIGYOCD", False)
                    NONYUNM1.Focus()
                End If
            Else
