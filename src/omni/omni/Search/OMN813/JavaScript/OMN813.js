...
function SIRCD_Search(e, str) {
    ...
    // 規定値のフォーカスセット
    if (str == "FROM2") {
        var elmNo = getElmNo('btnSIRCDFROM2');
        var elmNo2 = getElmNo('SIRCDFROM2');
        hidstrFocusSet(elmNo, elmNo2, '0');
        var elm = getNextFocusElm(elmNo, 'cli');
        var elm2 = getNextFocusElmE(elmNo2, 'cli');
    } else {
        var elmNo = getElmNo('btnSIRCDTO2');
        var elmNo2 = getElmNo('SIRCDTO2');
        hidstrFocusSet(elmNo, elmNo2, '0');
        var elm = getNextFocusElm(elmNo, 'cli');
        var elm2 = getNextFocusElmE(elmNo2, 'cli');
    }
    ...
    if (retval != null) {
        if (str == "FROM2") {
            if (btnAJOne('btnAJSIRNMR01') == false) {
                elm.focus();
            }
        } else {
            if (btnAJOne('btnAJSIRNMR02') == false) {
                elm.focus();
            }
        }
    } else {
