...
#Region "オーバーライドするメソッド"
    ...
    Protected Overrides Sub mSubBtnAJSearch()
        ...
        With mprg.mwebIFDataTable        '検索
            ...
            Else
                '取得可否チェック
                With CType(mprg.gmodel, ClsOMN105).gcol_H
                    If .strDELKBN = "1" Then
                        '削除済み時
                        Select Case mGet更新区分()
                            Case em更新区分.新規
            ...
            '値を退避
            Dim oCopy_H As New ClsOMN105.ClsCol_H
            ClsChkStringUtil.gSubDeepCopy(oCopy_H, CType(mprg.gmodel, ClsOMN105).gcol_H)
            CType(mprg.gmodel, ClsOMN105).gcopy_H = oCopy_H
    ...
    Protected Overrides Sub mSubGetText()
        With CType(mprg.gmodel, ClsOMN105).gcol_H
            .strBUNRUIDCD = BUNRUIDCD.Text                            '大分類コード

            .strBUNRUIDNM = BUNRUIDNM.Text                            '大分類名
    ...
    Protected Overrides Sub mSubSetText()
        With CType(mprg.gmodel, ClsOMN105).gcol_H
            'TODO 個別修正箇所
            BUNRUIDCD.Text = .strBUNRUIDCD                            '大分類コード

            BUNRUIDNM.Text = .strBUNRUIDNM                            '大分類名
    ...
    Protected Overrides Function mBlnChkInput(ByVal arrErrMsg As ClsErrorMessageList) As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN105)
            '変更あり/なし

            Dim oCopy_H = CType(mprg.gmodel, ClsOMN105).gcopy_H

            '変更の有無をチェックし、
            'If mGet更新区分() = em更新区分.変更 Then

            '    If oCopy_H Is Nothing Then
            '        oCopy_H = New ClsOMN105.ClsCol_H
            '    End If
    ...
    Protected Overrides Function mBln表示用にフォーマット() As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN105)
            With .gcol_H
            .strBUNRUIDNM = .strBUNRUIDNM                                                 '大分類名
...
#End Region
    ...
    Protected Overrides Sub mSubCreateWebIFData()
        ...
        With mprg.mwebIFDataTable
            .gSubAdd(BUNRUIDCD.ClientID,"BUNRUIDCD", 0, "numzero__2_", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(btnBUNRUIDCD.ClientID,"btnBUNRUIDCD", 0, "", "", "", "", "", "keyElm", "1", "0")
            .gSubAdd(btnSearch.ClientID,"btnSearch", 0, "", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(BUNRUIDNM.ClientID,"BUNRUIDNM", 0, "bytecount__10_", "", "", "", "", "mainElm", "1", "1")
