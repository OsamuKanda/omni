	...
	if (bshift == false) {
		if (nCode == 9) {
			...
			// TABキーのみの場合
				...
				case btnMode[1]: elm = document.getElementById(btnMode[1]); break;
...
// -------------------------------------------------------------
// モード変更によるエレメントをすべて変更します
// -------------------------------------------------------------
// arg[0] : bool: true/false
// return :     :
// -------------------------------------------------------------
function elementView(bMode) {
	if (bMode) {
	    document.getElementById(modeCANGE[0][0]).style.display = '';
	    document.getElementById(modeCANGE[1][0]).style.display = '';
	    document.getElementById(modeCANGE[2][0]).style.display = '';
	    document.getElementById(modeCANGE[3][0]).style.display = '';
	} else {
	    document.getElementById(modeCANGE[0][0]).style.display = 'none';
	    document.getElementById(modeCANGE[1][0]).style.display = 'none';
	    document.getElementById(modeCANGE[2][0]).style.display = 'none';
	    document.getElementById(modeCANGE[3][0]).style.display = 'none';
	}
}

// -------------------------------------------------------------
// モード変更によるエレメント状態に合わせて変更します
// -------------------------------------------------------------
// arg[0] : num : nMode
// return :     :
// -------------------------------------------------------------
function elementViewCng(nMode) {
	for (i = 0; i < modeCANGE.length; i++) {
		elm = document.getElementById(modeCANGE[i][0]);
		elm.style.visibility = modeCANGE[i][nMode];
	}
}
...
function ZIPCODE_Search(e, str) {

	var url = "../../../Search/OMN802/Contents/OMN802.aspx";
	var mode = "Search";
	var nWidth = "727px";
	var nHeight = "609px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	if (retval != null) {
		for (i = 0; i < objclient.length; i++) {
			if (objclient[i][1] == "ZIPCODE" + str) {
				elm = document.getElementById(objclient[i][0]);
				elm.value = retval[1].rtrim();
				elm.style.backgroundColor = objelmdata.Color('FocusOFF');
			}
		}
		elm = document.getElementById(ZIPIDNO);
		elm.value = retval[0];

	}

	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnZIPCODE' + str);
	var elmNo2 = getElmNo('ZIPCODE' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJZIPCODE' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function SEIKYUSAKICD_Search(e, str) {
	...
	// 規定値のフォーカスセット
	var elmNo2 = getElmNo('SEIKYUSAKICD' + str);
	var elm = getNextFocusElm(elmNo2 + 1);
	elm = idConv(elm.id);
	var elmNo = getElmNo(elm);
	hidstrFocusSet(elmNo, elmNo2, '0');
	elm = getNextFocusElm(elmNo, 'cli');
...
function KIGYOCD_Search(e, str) {

	var url = "../../../Search/OMN803/Contents/OMN803.aspx";
	var mode = "Search";
	var nWidth = "379px";
	var nHeight = "630px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnKIGYOCD' + str);
	var elmNo2 = getElmNo('KIGYOCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJKIGYONM' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function AREACD_Search(e, str) {

	var url = "../../../Search/OMN804/Contents/OMN804.aspx";
	var mode = "Search";
	var nWidth = "319px";
	var nHeight = "588px";
	var option = "dialogWidth=" + nWidth + ";dialogHeight=" + nHeight + ";center:1;status:no;scroll:no;resizable:no;";
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnAREACD' + str);
	var elmNo2 = getElmNo('AREACD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
	e.disabled = false;
	if (retval != null) {
	    if (btnAJOne('btnAJAREANM' + str) == false) {
	        elm.focus();
	    }
	} else {
	    elm2.focus();
...
function EIGYOTANTCD_Search(e, str) {
	...
	if (retval != null) {
		for (i = 0; i < objclient.length; i++) {
			if (objclient[i][1] == "EIGYOTANTCD" + str) {
				elm = document.getElementById(objclient[i][0]);
				elm.value = retval[0].rtrim();
				elm.style.backgroundColor = objelmdata.Color('FocusOFF');
			}
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnEIGYOTANTCD' + str);
	var elmNo2 = getElmNo('EIGYOTANTCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
	var elm = getNextFocusElm(elmNo, 'cli');
	var elm2 = getNextFocusElmE(elmNo2, 'cli');
	//modori値の処理
...
function SEIKYUCD_Search(e, str) {
    ...
    // 規定値のフォーカスセット
    var elmNo = getElmNo('btnSEIKYUCD' + str);
    var elmNo2 = getElmNo('NONYUCD' + str);
    hidstrFocusSet(elmNo, elmNo2, '0');
    var elm = getNextFocusElm(elmNo, 'cli');
    var elm2 = getNextFocusElmE(elmNo2, 'cli');
