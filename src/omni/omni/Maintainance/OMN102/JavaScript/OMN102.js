...
function JIGYOCD_Search(e, str) {

	var url = "../../../Search/OMN818/Contents/OMN818.aspx";
	var mode = "Search";
	var nWidth = "300px";
	var nHeight = "567px";
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnJIGYOCD' + str);
...
function HOSHUTANTCD_Search(e, str) {
    ...
    if (retval != null) {
        for (i = 0; i < objclient.length; i++) {
            if (objclient[i][1] == "HOSHUTANTCD" + str) {
                elm = document.getElementById(objclient[i][0]);
                elm.value = retval[0].rtrim();
                elm.style.backgroundColor = objelmdata.Color('FocusOFF');
            }
    ...
    // 規定値のフォーカスセット
    var elmNo = getElmNo('btnHOSHUTANTCD' + str);
    var elmNo2 = getElmNo('HOSHUTANTCD' + str);
    hidstrFocusSet(elmNo, elmNo2, '0');
    var elm = getNextFocusElm(elmNo, 'cli');
    var elm2 = getNextFocusElmE(elmNo2, 'cli');
    //modori値の処理
