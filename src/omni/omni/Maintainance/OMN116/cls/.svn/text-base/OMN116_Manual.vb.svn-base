...
#Region "オーバーライドするメソッド"
    ...
    Protected Overrides Sub mSubBtnAJSearch()
        ...
        With mprg.mwebIFDataTable        '検索
            ...
            Else
                '取得可否チェック
                With CType(mprg.gmodel, ClsOMN116).gcol_H
                    If .strDELKBN = "1" Then
                        '削除済み時
                        Select Case mGet更新区分()
                            Case em更新区分.新規
            ...
            '値を退避
            Dim oCopy_H As New ClsOMN116.ClsCol_H
            ClsChkStringUtil.gSubDeepCopy(oCopy_H, CType(mprg.gmodel, ClsOMN116).gcol_H)
            CType(mprg.gmodel, ClsOMN116).gcopy_H = oCopy_H
    ...
    Protected Overrides Sub mSubGetText()
        With CType(mprg.gmodel, ClsOMN116).gcol_H
            .strTAISHOCD = TAISHOCD.Text                              '対処コード

            .strTAISHONAIYO = TAISHONAIYO.Text                        '対処内容
    ...
    Protected Overrides Sub mSubSetText()
        With CType(mprg.gmodel, ClsOMN116).gcol_H
            'TODO 個別修正箇所
            TAISHOCD.Text = .strTAISHOCD                              '対処コード

            TAISHONAIYO.Text = .strTAISHONAIYO                        '対処内容
    ...
    Protected Overrides Function mBlnChkInput(ByVal arrErrMsg As ClsErrorMessageList) As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN116)
    ...
    Protected Overrides Function mBln表示用にフォーマット() As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN116)
            With .gcol_H
            .strTAISHONAIYO = .strTAISHONAIYO                                             '対処内容
...
#End Region
    ...
    Protected Overrides Sub mSubCreateWebIFData()
        ...
        With mprg.mwebIFDataTable
            .gSubAdd(TAISHOCD.ClientID,"TAISHOCD", 0, "numzero__4_", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(btnTAISHOCD.ClientID,"btnTAISHOCD", 0, "", "", "", "", "", "keyElm", "1", "0")
            .gSubAdd(btnSearch.ClientID,"btnSearch", 0, "", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(TAISHONAIYO.ClientID,"TAISHONAIYO", 0, "!bytecount__100_", "", "", "", "", "mainElm", "1", "1")
