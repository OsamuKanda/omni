...
function BUNRUICCD_Search(e, str) {

	var url = "../../../Search/OMN817/Contents/OMN817.aspx";
	var mode = "Search";
	var nWidth = "300px";
	var nHeight = "567px";
	...
	if (retval != null) {
		for (i = 0; i < objclient.length; i++) {
			if (objclient[i][1] == "BUNRUICCD" + str) {
				elm = document.getElementById(objclient[i][0]);
	...
	// 規定値のフォーカスセット
	var elmNo = getElmNo('btnBUNRUICCD' + str);
	var elmNo2 = getElmNo('BUNRUICCD' + str);
	hidstrFocusSet(elmNo, elmNo2, '0');
