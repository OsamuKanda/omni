...
Partial Public Class OMN115Dao(Of T)
    ''' <summary>
    ''' 追加用SQL取得
    ''' </summary>
    ''' <param name="o"></param>
    ...
    Public Overrides Function getSQLInsert(ByVal o As T) As String
        ...
        With mclsCol_H
            'SQL
            strSQL.Append(" INSERT INTO DM_GENIN")
            strSQL.Append("(")
            strSQL.Append(" GENINCD")                                           '原因コード
            strSQL.Append(",GENINNAIYO")                                        '原因内容
            ...
            strSQL.Append(ClsDbUtil.get文字列値(.strGENINCD))                   '原因コード
            strSQL.Append("," & ClsDbUtil.get文字列値(.strGENINNAIYO))          '原因内容
    ...
    Public Overrides Function getSQLDelete(ByVal o As T) As String
        With o.gcol_H
            ...
            strSQL.Append(" WHERE DM_GENIN.GENINCD= '" & .strGENINCD & "'")                           '原因コード
    ...
    Public Overrides Function getSQLUpdate(ByVal o As T) As String
        ...
        With mclsCol_H
            Dim strSQL As New StringBuilder
            strSQL.Append("UPDATE DM_GENIN")
            strSQL.Append("   SET GENINNAIYO      = " & ClsDbUtil.get文字列値(.strGENINNAIYO))         '原因内容
    ...
    Public Overrides Function getSQLSelect(ByVal o As T) As String
        With o.gcol_H
            ...
            strSQL.Append("  DM_GENIN.GENINCD AS GENINCD ")
            strSQL.Append(", DM_GENIN.GENINNAIYO AS GENINNAIYO ")

            strSQL.Append(", DM_GENIN.DELKBN ")                                           '無効区分
            strSQL.Append(", DM_GENIN.UDTTIME1 ")                                         '新規更新日時
            strSQL.Append(", DM_GENIN.UDTUSER1 ")                                         '新規更新ユーザ
            strSQL.Append(", DM_GENIN.UDTPG1 ")                                           '新規更新機能
            '抽出条件
            strSQL.Append("  FROM ")
            strSQL.Append("  DM_GENIN ")                                                  'ヘッダ
            strSQL.Append("WHERE DM_GENIN.GENINCD = '" & .strGENINCD & "' ")                          '原因コード
            'If o.更新区分 <> em更新区分.新規 Then
            '    strSQL.Append("   AND DM_GENIN.DELKBN ='0'")
            'End If
    ...
    Protected Overrides Sub setTableTo(ByVal dt As System.Data.DataTable, ByVal o As T)
        With o.gcol_H
            Dim r = dt.Rows(0)
            .strGENINCD = r("GENINCD").ToString             '原因コード
            .strGENINNAIYO = r("GENINNAIYO").ToString       '原因内容
