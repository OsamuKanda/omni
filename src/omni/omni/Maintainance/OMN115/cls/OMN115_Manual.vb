...
#Region "オーバーライドするメソッド"
    ...
    Protected Overrides Sub mSubBtnAJSearch()
        ...
        With mprg.mwebIFDataTable        '検索
            ...
            Else
                '取得可否チェック
                With CType(mprg.gmodel, ClsOMN115).gcol_H
                    If .strDELKBN = "1" Then
                        '削除済み時
                        Select Case mGet更新区分()
                            Case em更新区分.新規
            ...
            '値を退避
            Dim oCopy_H As New ClsOMN115.ClsCol_H
            ClsChkStringUtil.gSubDeepCopy(oCopy_H, CType(mprg.gmodel, ClsOMN115).gcol_H)
            CType(mprg.gmodel, ClsOMN115).gcopy_H = oCopy_H
    ...
    Protected Overrides Sub mSubGetText()
        With CType(mprg.gmodel, ClsOMN115).gcol_H
            .strGENINCD = GENINCD.Text                                '原因コード

            .strGENINNAIYO = GENINNAIYO.Text                          '原因内容
    ...
    Protected Overrides Sub mSubSetText()
        With CType(mprg.gmodel, ClsOMN115).gcol_H
            'TODO 個別修正箇所
            GENINCD.Text = .strGENINCD                                '原因コード

            GENINNAIYO.Text = .strGENINNAIYO                          '原因内容
    ...
    Protected Overrides Function mBlnChkInput(ByVal arrErrMsg As ClsErrorMessageList) As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN115)
    ...
    Protected Overrides Function mBln表示用にフォーマット() As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN115)
            With .gcol_H
            .strGENINNAIYO = .strGENINNAIYO                                               '原因内容
...
#End Region
    ...
    Protected Overrides Sub mSubCreateWebIFData()
        ...
        With mprg.mwebIFDataTable
            .gSubAdd(GENINCD.ClientID,"GENINCD", 0, "numzero__4_", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(btnGENINCD.ClientID,"btnGENINCD", 0, "", "", "", "", "", "keyElm", "1", "0")
            .gSubAdd(btnSearch.ClientID,"btnSearch", 0, "", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(GENINNAIYO.ClientID,"GENINNAIYO", 0, "!bytecount__100_", "", "", "", "", "mainElm", "1", "1")
