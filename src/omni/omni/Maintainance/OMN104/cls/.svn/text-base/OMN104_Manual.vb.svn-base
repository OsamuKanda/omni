...
#Region "オーバーライドするメソッド"
    ...
    Protected Overrides Sub mSubBtnAJSearch()
        ...
        With mprg.mwebIFDataTable        '検索
            ...
            Else
                '取得可否チェック
                With CType(mprg.gmodel, ClsOMN104).gcol_H
                    If .strDELKBN = "1" Then
                        '削除済み時
                        Select Case mGet更新区分()
                            Case em更新区分.新規
            ...
            '値を退避
            Dim oCopy_H As New ClsOMN104.ClsCol_H
            ClsChkStringUtil.gSubDeepCopy(oCopy_H, CType(mprg.gmodel, ClsOMN104).gcol_H)
            CType(mprg.gmodel, ClsOMN104).gcopy_H = oCopy_H
    ...
    Protected Overrides Sub mSubGetText()
        With CType(mprg.gmodel, ClsOMN104).gcol_H
            .strGINKOCD = GINKOCD.Text                                '銀行コード

            .strGINKONM = GINKONM.Text                                '銀行名称
    ...
    Protected Overrides Sub mSubSetText()
        With CType(mprg.gmodel, ClsOMN104).gcol_H
            'TODO 個別修正箇所
            GINKOCD.Text = .strGINKOCD                                '銀行コード

            GINKONM.Text = .strGINKONM                                '銀行名称
    ...
    Protected Overrides Function mBlnChkInput(ByVal arrErrMsg As ClsErrorMessageList) As Boolean
        ...
        With CType(mprg.gmodel, ClsOMN104)
            '変更あり/なし

            Dim oCopy_H = CType(mprg.gmodel, ClsOMN104).gcopy_H

            '変更の有無をチェックし、
            'If mGet更新区分() = em更新区分.変更 Then

            '    If oCopy_H Is Nothing Then
            '        oCopy_H = New ClsOMN104.ClsCol_H
            '    End If
...
#End Region
    ...
    Protected Overrides Sub mSubCreateWebIFData()
        ...
        With mprg.mwebIFDataTable
            .gSubAdd(GINKOCD.ClientID,"GINKOCD", 0, "numzero__3_", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(btnGINKOCD.ClientID,"btnGINKOCD", 0, "", "", "", "", "", "keyElm", "1", "0")
            .gSubAdd(btnSearch.ClientID,"btnSearch", 0, "", "", "", "", "", "keyElm", "1", "1")
            .gSubAdd(GINKONM.ClientID,"GINKONM", 0, "bytecount__30_", "", "", "", "", "mainElm", "1", "1")
