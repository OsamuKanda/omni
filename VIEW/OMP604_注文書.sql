...
--OMP604 注文書
-------------------------------------------------------------------------------
CREATE OR REPLACE FORCE VIEW "OMNI"."V_OMP604" ("EDANUM", "発注事業所CD", "発注番号", "仕入先CD", "仕入先名1", "仕入先名2", "先方担当者", "発注日", "事業所名", "郵便番号", "住所1", "住所2", "電話番号", "FAX番号", "発注担当者CD", "担当者名", "行番号", "分類CD", "分類名", "規格CD", "規格名", "数量", "単位CD", "単位名", "納入場所区分", "納入場所", "納期日付", "納期区分", "納期区分名", "物件名", "工事予定日", "物件番号", "備考", "備考１", "備考２", "LOGINID") AS 
  SELECT
              ROWNUM  AS EDANUM
			...
			, (CASE WHEN DT_HACCHUH.HACCHUYMD < DM_JIGYO.NEWJIGYOYMD THEN DM_JIGYO.OLD_JIGYONM ELSE DM_JIGYO.JIGYONM END) AS 事業所名
			, (CASE WHEN DT_HACCHUH.HACCHUYMD < DM_JIGYO.NEWJIGYOYMD THEN DM_JIGYO.OLD_ZIPCODE ELSE DM_JIGYO.ZIPCODE END) AS 郵便番号
			, (CASE WHEN DT_HACCHUH.HACCHUYMD < DM_JIGYO.NEWJIGYOYMD THEN DM_JIGYO.OLD_ADD1 ELSE DM_JIGYO.ADD1 END) AS 住所1
			, (CASE WHEN DT_HACCHUH.HACCHUYMD < DM_JIGYO.NEWJIGYOYMD THEN DM_JIGYO.OLD_ADD2 ELSE DM_JIGYO.ADD2 END) AS 住所2
			, (CASE WHEN DT_HACCHUH.HACCHUYMD < DM_JIGYO.NEWJIGYOYMD THEN DM_JIGYO.OLD_TELNO ELSE DM_JIGYO.TELNO END) AS 電話番号
			, (CASE WHEN DT_HACCHUH.HACCHUYMD < DM_JIGYO.NEWJIGYOYMD THEN DM_JIGYO.OLD_FAXNO ELSE DM_JIGYO.FAXNO END) AS FAX番号
			-- ↑2023/08/21 UPDATE TC KANDA 請求日が2023年9月30日までのものは旧の事業所情報を表示
