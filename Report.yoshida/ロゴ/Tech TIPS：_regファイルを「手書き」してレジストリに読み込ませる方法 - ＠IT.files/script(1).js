var _nc_ttnews111125_params = (function() {
	var result = {};
	var crossdomain;
...
var _nc_ttnews111125_clicked = function (url) {
	new Image().src = 'http://connect.newzia.jp/channels/181/clicked'
		+ '?url=' + encodeURIComponent(url)
		+ '&page_url=' + encodeURIComponent(_nc_ttnews111125_params.url)
		+ '&ref=' + encodeURIComponent(_nc_ttnews111125_params.referrer)
		+ '&' + Math.random();
}
;
var _nc_ttnews111125_callback = function (response) {
	var params = _nc_ttnews111125_params;
	(function (contents_length) {
	new Image().src = 'http://connect.newzia.jp/channels/181/viewed'
		+ '?url=' + encodeURIComponent(_nc_ttnews111125_params.url)
		+ '&ref=' + encodeURIComponent(_nc_ttnews111125_params.referrer)
		+ '&contents_length=' + contents_length
		+ '&' + Math.random();
}
)(response.length);
	var newziaConnect = document.getElementById('newzia_connect_ttnews111125');
var getMedianamettnews111125 = function() {
	if (document.URL.match('/fwin2k/')) {
		medianame = 'tt_atit_rc_windows';
	} else if (document.URL.match('/fdotnet/')) {
		medianame = 'tt_atit_rc_dotnet';
	} else if (document.URL.match('/fsys/')) {
		medianame = 'tt_atit_rc_system';
	} else if (document.URL.match('/fserver/')) {
		medianame = 'tt_atit_rc_serverstorage';
	} else if (document.URL.match('/fwcr/design/')) {
		medianame = 'tt_atit_rc_d89';
	} else if (document.URL.match('/fwcr/')) {
		medianame = 'tt_atit_rc_richclient';
	} else if (document.URL.match('/fcoding/')) {
		medianame = 'tt_atit_rc_codingedge';
	} else if (document.URL.match('/fjava/')) {
		medianame = 'tt_atit_rc_java';
	} else if (document.URL.match('/fdb/')) {
		medianame = 'tt_atit_rc_database';
	} else if (document.URL.match('/fxml/')) {
		medianame = 'tt_atit_rc_xml';
	} else if (document.URL.match('/flinux/')) {
		medianame = 'tt_atit_rc_linux';
	} else if (document.URL.match('/fnetwork/')) {
		medianame = 'tt_atit_rc_ipnetwork';
	} else if (document.URL.match('/fsecurity/')) {
		medianame = 'tt_atit_rc_security';
	} else if (document.URL.match('/frfid/')) {
		medianame = 'tt_atit_rc_rfid';
	} else if (document.URL.match('/fkaiketsu/')) {
		medianame = 'tt_atit_rc_fkaiketsu';
	} else if (document.URL.match('/im/cits/')) {
		medianame = 'tt_im_rc_cits';
	} else if (document.URL.match('/im/cop/')) {
		medianame = 'tt_im_rc_cop';
	} else if (document.URL.match('/im/carc/')) {
		medianame = 'tt_im_rc_carc';
	} else if (document.URL.match('/im/cpm/')) {
		medianame = 'tt_im_rc_cpm';
	} else if (document.URL.match('/im/cbp/')) {
		medianame = 'tt_im_rc_cbp';
	} else if (document.URL.match('/im/cap/')) {
		medianame = 'tt_im_rc_cap';
	} else if (document.URL.match('/im/ces/')) {
		medianame = 'tt_im_rc_webmarketing';
	} else if (document.URL.match('/fembedded/')) {
		medianame = 'tt_monoist_rc_fembedded';
	} else if (document.URL.match('/fpro/')) {
		medianame = 'tt_monoist_rc_fpro';
	} else {
		medianame = 'tt_atit_rc_newzia';
	}
	return medianame;
}

var ttnews111125temp = '';
ttnews111125temp += "<div id=\"newzia_connect_ttnews111125\" class=\"btmlist\"><ul>";

for (i = 0; i < response.length; i++) {
	ttnews111125temp += "<li><a onclick=\"var s=s_gi('itmediacojpglobal'); s.eVar21='" + getMedianamettnews111125() + "'; s.tl(this,'o','" + getMedianamettnews111125() +"');\" href=\"" + response[i].url + "\" ontouchstart=\"_nc_ttnews111125_clicked(this.href)\" onmousedown=\"_nc_ttnews111125_clicked(this.href)\">" + response[i].title + "</a></li>";
}

ttnews111125temp += "</ul></div>";
newziaConnect.innerHTML = ttnews111125temp;
...
(function () {
	...
	var callNewziaConnect = function () {
		...
		script.src = 'http://connect.newzia.jp/channels/181/' + params.digest + '/contents.json?'
	...
	if (!document.getElementById('newzia_connect_ttnews111125')) {
		document.write('<div id="newzia_connect_ttnews111125"></div>');
