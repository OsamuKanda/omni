var _nc_ttwproi111125_params = (function() {
	var result = {};
	var crossdomain;
	...
	if (!window.parent || window === window.parent) {
		...
		try {
			crossdomain = !("document" in parent) || !parent.document;
		} catch (_error) {
			crossdomain = true;
		}
		if (!crossdomain) {
			if (document.referrer === parent.document.URL) {
				result.url = parent.document.URL;
				result.ref = parent.document.referrer;
			} else {
				result.url = document.URL;
				result.ref = document.referrer;
			}
		} else {
			if (document.URL.split('/')[2] !== document.referrer.split('/')[2]) {
...
;
	result.digest = md5.hex(result.url);


	return result;
})();
var _nc_ttwproi111125_clicked = function (url) {
	new Image().src = 'http://connect.newzia.jp/channels/179/clicked'
		+ '?url=' + encodeURIComponent(url)
		+ '&page_url=' + encodeURIComponent(_nc_ttwproi111125_params.url)
		+ '&ref=' + encodeURIComponent(_nc_ttwproi111125_params.referrer)
		+ '&' + Math.random();
}
;
var _nc_ttwproi111125_callback = function (response) {
	var params = _nc_ttwproi111125_params;
	(function (contents_length) {
	new Image().src = 'http://connect.newzia.jp/channels/179/viewed'
		+ '?url=' + encodeURIComponent(_nc_ttwproi111125_params.url)
		+ '&ref=' + encodeURIComponent(_nc_ttwproi111125_params.referrer)
		+ '&contents_length=' + contents_length
		+ '&' + Math.random();
}
)(response.length);
	var newziaConnect = document.getElementById('newzia_connect_ttwproi111125');
var getMedianamettwproi111125 = function() {
	if (document.URL.match('/fwin2k/')) {
		medianame = 'ROI_tt_atit_wp_windows';
	} else if (document.URL.match('/fdotnet/')) {
		medianame = 'ROI_tt_atit_wp_dotnet';
	} else if (document.URL.match('/fsys/')) {
		medianame = 'ROI_tt_atit_wp_system';
	} else if (document.URL.match('/fserver/')) {
		medianame = 'ROI_tt_atit_wp_serverstorage';
	} else if (document.URL.match('/fwcr/design/')) {
		medianame = 'ROI_tt_atit_wp_d89';
	} else if (document.URL.match('/fwcr/')) {
		medianame = 'ROI_tt_atit_wp_richclient';
	} else if (document.URL.match('/fcoding/')) {
		medianame = 'ROI_tt_atit_wp_codingedge';
	} else if (document.URL.match('/fjava/')) {
		medianame = 'ROI_tt_atit_wp_java';
	} else if (document.URL.match('/fdb/')) {
		medianame = 'ROI_tt_atit_wp_database';
	} else if (document.URL.match('/fxml/')) {
		medianame = 'ROI_tt_atit_wp_xml';
	} else if (document.URL.match('/flinux/')) {
		medianame = 'ROI_tt_atit_wp_linux';
	} else if (document.URL.match('/fnetwork/')) {
		medianame = 'ROI_tt_atit_wp_ipnetwork';
	} else if (document.URL.match('/fsecurity/')) {
		medianame = 'ROI_tt_atit_wp_security';
	} else if (document.URL.match('/frfid/')) {
		medianame = 'ROI_tt_atit_wp_rfid';
	} else if (document.URL.match('/fkaiketsu/')) {
		medianame = 'ROI_tt_atit_wp_fkaiketsu';
	} else if (document.URL.match('/im/cits/')) {
		medianame = 'ROI_tt_im_wp_cits';
	} else if (document.URL.match('/im/cop/')) {
		medianame = 'ROI_tt_im_wp_cop';
	} else if (document.URL.match('/im/carc/')) {
		medianame = 'ROI_tt_im_wp_carc';
	} else if (document.URL.match('/im/cpm/')) {
		medianame = 'ROI_tt_im_wp_cpm';
	} else if (document.URL.match('/im/cbp/')) {
		medianame = 'ROI_tt_im_wp_cbp';
	} else if (document.URL.match('/im/cap/')) {
		medianame = 'ROI_tt_im_wp_cap';
	} else if (document.URL.match('/im/ces/')) {
		medianame = 'ROI_tt_im_wp_webmarketing';
	} else if (document.URL.match('/fembedded/')) {
		medianame = 'ROI_tt_monoist_wp_fembedded';
	} else if (document.URL.match('/fpro/')) {
		medianame = 'ROI_tt_monoist_wp_fpro';
	} else if (document.URL.match('/enterprise/')) {
		medianame = 'ROI_tt_ent_wp';
	} else if (document.URL.match('/news/')) {
		medianame = 'ROI_tt_news_wp';
	} else {
		medianame = 'ROI_tt_atit_wp_newzia';
	}
	return medianame;
}

var ttwproi111125temp = '';
ttwproi111125temp += "<div id=\"newzia_connect_ttwproi111125\" class=\"btmlist newziaConnectAPI\"><ul>";

for (i = 0; i < response.length; i++) {
	ttwproi111125temp += "<li><a onclick=\"var s=s_gi('itmediacojpglobal'); s.eVar21='" + getMedianamettwproi111125() + "'; s.tl(this,'o','" + getMedianamettwproi111125() +"');\" href=\"" + response[i].url + "\" ontouchstart=\"_nc_ttwproi111125_clicked(this.href)\" onmousedown=\"_nc_ttwproi111125_clicked(this.href)\">" + response[i].title + "</a></li>";
}

ttwproi111125temp += "</ul></div>";
newziaConnect.innerHTML = ttwproi111125temp;
};
(function () {
	var params = _nc_ttwproi111125_params;
	...
	var callNewziaConnect = function () {
		...
		script.src = 'http://connect.newzia.jp/channels/179/' + params.digest + '/contents.json?'
	...
	if (!document.getElementById('newzia_connect_ttwproi111125')) {
		document.write('<div id="newzia_connect_ttwproi111125"></div>');
