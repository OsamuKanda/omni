var _nc_ttwp111125_params = (function() {
	var result = {};
	var crossdomain;
...
var _nc_ttwp111125_clicked = function (url) {
	new Image().src = 'http://connect.newzia.jp/channels/180/clicked'
		+ '?url=' + encodeURIComponent(url)
		+ '&page_url=' + encodeURIComponent(_nc_ttwp111125_params.url)
		+ '&ref=' + encodeURIComponent(_nc_ttwp111125_params.referrer)
		+ '&' + Math.random();
}
;
var _nc_ttwp111125_callback = function (response) {
	var params = _nc_ttwp111125_params;
	(function (contents_length) {
	new Image().src = 'http://connect.newzia.jp/channels/180/viewed'
		+ '?url=' + encodeURIComponent(_nc_ttwp111125_params.url)
		+ '&ref=' + encodeURIComponent(_nc_ttwp111125_params.referrer)
		+ '&contents_length=' + contents_length
		+ '&' + Math.random();
}
)(response.length);
	var newziaConnect = document.getElementById('newzia_connect_ttwp111125');
var getMedianamettwp111125 = function() {
...
var ttwp111125temp = '';
ttwp111125temp += "<div id=\"newzia_connect_ttwp111125\" class=\"btmlist newziaConnectAPI\"><ul>";

for (i = 0; i < response.length; i++) {
	ttwp111125temp += "<li><a onclick=\"var s=s_gi('itmediacojpglobal'); s.eVar21='" + getMedianamettwp111125() + "'; s.tl(this,'o','" + getMedianamettwp111125() +"');\" href=\"" + response[i].url + "\" ontouchstart=\"_nc_ttwp111125_clicked(this.href)\" onmousedown=\"_nc_ttwp111125_clicked(this.href)\">" + response[i].title + "</a></li>";
}

ttwp111125temp += "</ul></div>";
newziaConnect.innerHTML = ttwp111125temp;
...
(function () {
	...
	var callNewziaConnect = function () {
		...
		script.src = 'http://connect.newzia.jp/channels/180/' + params.digest + '/contents.json?'
	...
	if (!document.getElementById('newzia_connect_ttwp111125')) {
		document.write('<div id="newzia_connect_ttwp111125"></div>');
